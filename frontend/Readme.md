...
`/users/login` - Авторизация пользователя и проверка состояния пользователя (авторизован/не авторизован)

`/users/register` - Регистрация нового пользователя

`/users/:userId/avatar` - Загрузка аватарки пользователя

### Предложения по аренде

`/offers` - Получение списка предложений и создание нового предложения

`/offers/:offerId` - Детальная информация, редактирование и удаление определенного предложения

`/offers/:offerId/preview` - Загрузка превью предложения

`/offers/favorites` - Получение избранных предложений

`/offers/favorites/:offerId` - Изменение статуса предложения

`/offers/premium`- Получение премиальных предложений для города

### Отзывы

`/reviews` - Добавление нового комментария

`/reviews/:offerId` - Получение списка отзывов по предложению
